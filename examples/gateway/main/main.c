...
LocoNetInterfaceHandle loconet_handle;
...
static void message_send_task(void *arg) {
    ...
    while (1) {
        loconet_message_send(loconet_handle, &raw);
...
void app_main(void) {
    gateway_led_init();
    loconet_handle = (LocoNetInterfaceHandle)loconet_uart_start(loconet_uart_config);
    ...
    if( loconet_receive_queue == NULL ) {
        ...
        loconet_interface_register_cb_read(loconet_handle, &loconet_read_callback);
    }

    
    loconet_interface_register_cb_write(loconet_handle, &loconet_write_callback);
